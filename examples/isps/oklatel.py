...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Oklatel CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://fiber.oklatel.net/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Oklatel page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Type `%address%` into the input field", "Type `%zip_code%` into the input field", "Press the 'Service Type' dropdown and press down key and press enter key", "Click the 'Go' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
...
if os.path.exists("examples/isps/results/oklatel_result.json"):
    with open("examples/isps/results/oklatel_result.json", "r") as f:
        try:
            data = json.load(f)
...
result = agent.run(
    ...
    session="oklatel",
    save_content_dir="examples/isps/save/oklatel"
)
...
with open("examples/isps/results/oklatel_result.json", "w") as f:
