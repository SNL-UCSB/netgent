...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Zirrus CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://getfiber.zirrus.com/front_end/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Zirrus page",
        ...
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Accept cookies first", "Type `%address%` into the input field", "Type `%zip_code%` into the input field", "Click on 'Home or Business' dropdown, down arrow key once, then enter key", "Click the 'Go' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        ...
        actions=["Scroll down on the page", "Click the 'Next' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="SERVICEABLE",
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available - (e.g. 'Excellent news! Zirrus Fiber Internet is expanding rapidly. Zirrus prioritizes fiber expansion in areas with the highest interest.')",
        triggers=["If you see 'Excellent news! Zirrus Fiber Internet is expanding rapidly. Zirrus prioritizes fiber expansion in areas with the highest interest.'"],
...
for i in range(15):
    ...
    try:
        result = agent.run(
            ...
            session="zirrus_robust",
            save_content_dir="examples/isps/save/zirrus",
            close_browser=True
        )
...
with open("examples/isps/results/zirrus_robust_results.json", "w") as f:
    json.dump(results, f, indent=2)

# Also update the main result repository
with open("examples/isps/results/zirrus_result.json", "w") as f:
