...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Franklin Telephone page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://www.franklintelephone.com/AddressSearch/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Franklin Telephone page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Scroll down a little bit", "Type `%address%` and `%zip_code%` into the input field", "Click the 'Go' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="SERVICEABLE",
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available - (e.g. 'Your address is not currently in our fiber service area.')",
        triggers=["If you see 'Your address is not currently in our fiber service area.'"],
        actions=["DON'T DO ANYTHING, JUST TERMINATE AT THIS POINT"],
...
if os.path.exists("examples/isps/results/franklintelephone_result.json"):
    with open("examples/isps/results/franklintelephone_result.json", "r") as f:
        try:
            data = json.load(f)
...
result = agent.run(
    ...
    session="franklintelephone",
    save_content_dir="examples/isps/save/franklintelephone"
)
...
with open("examples/isps/results/franklintelephone_result.json", "w") as f:
