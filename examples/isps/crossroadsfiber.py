...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Crossroads Fiber CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://www.crossroadsfiber.net/signup word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Crossroads Fiber page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Type `%address%` and `%zip_code%` in the input field", "Click the 'Find' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="SERVICEABLE",
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available - (e.g. 'No matching address. You may fill in our contact form and we will respond to you!')",
        triggers=["If you see 'No matching address. You may fill in our contact form and we will respond to you!'"],
        actions=["DON'T DO ANYTHING, JUST TERMINATE AT THIS POINT"],
...
if os.path.exists("examples/isps/results/crossroadsfiber_result.json"):
    with open("examples/isps/results/crossroadsfiber_result.json", "r") as f:
        try:
            data = json.load(f)
...
result = agent.run(
    ...
    session="crossroadsfiber",
    save_content_dir="examples/isps/save/crossroadsfiber"
)
...
with open("examples/isps/results/crossroadsfiber_result.json", "w") as f:
