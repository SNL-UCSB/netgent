...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Twin Valley page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://twinvalley.com/check-availability/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Twin Valley page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Close Pop Up", "Type `%address%` into the input field", "Type `%zip_code%` into the input field", "Click the 'Go' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="SERVICEABLE",
    ...
    StatePrompt(
        ...
        triggers=["Check the URL here: https://twinvalley.com/check-availability/#NoServicesAvailable"],
        actions=["TERMINATE AT THIS POINT"],
...
result = agent.run(
    ...
    session="twinvalley",
    save_content_dir="examples/isps/save/twinvalley"
)
...
with open("examples/isps/results/twinvalley_result.json", "w") as f:
