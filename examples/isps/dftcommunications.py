...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to DFT Communications CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://connect.dftcommunications.com/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the DFT Communications page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY",
                "Accept cookies first",
                "Type `%address%` into the input field",
                "Type `%zip_code%` into the input field",
                "Click on 'Service' dropdown, down arrow key once, then enter key",
                "Click the 'Go' button",
...
for i in range(15):
    ...
    try:
        result = agent.run(
            ...
            session="dftcommunications",
            save_content_dir="examples/isps/save/dftcommunications",
            close_browser=True
        )
...
with open("examples/isps/results/dftcommunications_robust_results.json", "w") as f:
    json.dump(results, f, indent=2)

# Also update the main result repository
with open("examples/isps/results/dftcommunications_result.json", "w") as f:
