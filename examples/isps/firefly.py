...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Firefly CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://register.fireflyva.com/front_end/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Firefly page",
...
if os.path.exists("examples/isps/results/firefly_result.json"):
    with open("examples/isps/results/firefly_result.json", "r") as f:
        try:
            data = json.load(f)
...
for i in range(15):
    try:
        ...
        result = agent.run(
            ...
            session="firefly",
            save_content_dir="examples/isps/save/firefly",
            close_browser=True
        )
...
with open("examples/isps/results/firefly_robust_results.json", "w") as f:
    json.dump(results, f, indent=2)

# Update the main result repository
with open("examples/isps/results/firefly_result.json", "w") as f:
