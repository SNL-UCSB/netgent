...
prompts = [
        StatePrompt(
            ...
            actions=["Go to https://www.truestreamfiber.com/new-truestreamer/#status word for word", "TERMINATE AT THIS POINT"],
        ),
        StatePrompt(
            name="ADDRESS_BAR",
            description="Enter address to check availability",
            triggers=["If you see 'AVAILABILITY AND STATUS'"],
            actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Click on the '123 Main St...' input field", "Type `%address%` into the address input field", "Press down key to select the first suggestion", "Press enter key to confirm selection", "Scroll down", "TERMINATE AT THIS POINT"],
        ),
        StatePrompt(
            name="SERVICEABLE",
...
with BQTDatabase() as db:
    # Fetching addresses for TrueStream service area
    rows = db.query("SELECT * FROM bqtplus.truestream_addresses LIMIT 1000")
    for row in rows:
...
try:
    with open("examples/isps/results/truestream_result.json", "r") as f:
...
result = agent.run(
    ...
    save_content_dir="examples/isps/save/truestream",
    session="truestream"
)
...
with open("examples/isps/results/truestream_result.json", "w") as f:
