...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Crossbeam CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://crossbeam.net/front_end/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Crossbeam page",
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available - (e.g. 'Unfortunately, your address is outside of Carroll EMC’s service territory, which is who we are partnered with/where we are currently serving.')",
        triggers=["If you see 'Unfortunately, your address is outside of Carroll EMC’s service territory, which is who we are partnered with/where we are currently serving.'"],
        actions=["DON'T DO ANYTHING, JUST TERMINATE AT THIS POINT"],
...
if os.path.exists("examples/isps/results/crossbeam_result.json"):
    with open("examples/isps/results/crossbeam_result.json", "r") as f:
        try:
            data = json.load(f)
...
for i in range(15):
    try:
        ...
        result = agent.run(
            ...
            session="crossbeam",
            save_content_dir="examples/isps/save/crossbeam",
            close_browser=True
        )
...
with open("examples/isps/results/crossbeam_robust_results.json", "w") as f:
    json.dump(results, f, indent=2)

# Update the main result repository
with open("examples/isps/results/crossbeam_result.json", "w") as f:
