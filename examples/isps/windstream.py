...
prompts = [
        StatePrompt(
            name="START",
            description="Navigate to the Windstream check availability page",
            triggers=["If it is on the chrome homescreen"],
            actions=["Go to https://buy.windstream.com/check-availability word for word", "TERMINATE AT THIS POINT"],
        ),
        StatePrompt(
            name="ADDRESS_BAR",
            ...
            actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Click on the address input field", "Type `%address%` into the address input field", "Press down key to select the first suggestion", "Press enter key to confirm selection", "Click the submit or continue button if available", "TERMINATE AT THIS POINT"],
        ),
        StatePrompt(
            name="Internet Available at Address on Windstream",
        ...
        StatePrompt(
            name="Internet Not Available at Address on Windstream",
...
with BQTDatabase() as db:
    # Fetching addresses for Windstream service area
    rows = db.query("SELECT * FROM bqtplus.windstream_addresses LIMIT 1000")
    for row in rows:
...
try:
    with open("examples/isps/results/windstream_result.json", "r") as f:
...
result = agent.run(
    ...
    save_content_dir="examples/isps/save/windstream",
    session="windstream"
)
...
with open("examples/isps/results/windstream_result.json", "w") as f:
