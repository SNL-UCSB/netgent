...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Aeneas CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://fiber.aeneas.com/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Aeneas page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Type `%address%` and `%zip_code%` into the input field", "Press the 'Enter' key", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="SERVICEABLE",
...
if os.path.exists("examples/isps/results/aeneas_result.json"):
    with open("examples/isps/results/aeneas_result.json", "r") as f:
        try:
            data = json.load(f)
...
result = agent.run(
    ...
    save_content_dir="examples/isps/save/aeneas",
    session="aeneas"
)
...
with open("examples/isps/results/aeneas_result.json", "w") as f:
