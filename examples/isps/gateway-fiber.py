...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Gateway Fiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://shop.gatewayfiber.com/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Gateway Fiber page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Type `%address%` into the input field", "Press down and enter", "Click submit to confirm selection", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
    ...
    StatePrompt(
        ...
        triggers=["If you see 'Socket is in your neighborhood. Order now!'"],
        actions=[
...
try:
    with open("examples/isps/results/gateway-fiber_result.json", "r") as f:
...
result = agent.run(
    ...
    save_content_dir="examples/isps/save/gateway-fiber",
    session="gateway-fiber"
)
...
with open("examples/isps/results/gateway-fiber_result.json", "w") as f:
