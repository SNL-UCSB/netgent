...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Ignite Broadband CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://join.ignitebroadband.com/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Ignite Broadband page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Accept cookies first", "Type `%address%` into the input field", "Type `%zip_code%` into the input field", "Click the dropdown, Press down key, and press enter key", "Click the 'Go' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        ...
        actions=["Scroll to the bottom", "Click the 'Next' button", "DON'T DO ANYTHING, JUST TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="SERVICEABLE",
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available - (e.g. 'Unavailable at this location.')",
        triggers=["If you see 'Unavailable at this location.'"],
        actions=["DON'T DO ANYTHING, JUST TERMINATE AT THIS POINT"],
...
if os.path.exists("examples/isps/results/ignitebroadband_result.json"):
    with open("examples/isps/results/ignitebroadband_result.json", "r") as f:
        try:
            data = json.load(f)
...
result = agent.run(
    ...
    session="ignitebroadband",
    save_content_dir="examples/isps/save/ignitebroadband"
)
...
with open("examples/isps/results/ignitebroadband_result.json", "w") as f:
