...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Elevate Fiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://join.elevatefiber.com/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
    ...
    StatePrompt(
        ...
        triggers=["If you see 'Elevate is in your neighborhood. Order now!'"],
        actions=[
...
with BQTDatabase() as db:
    # Fetching addresses for Elevate Fiber service area
...
try:
    with open("examples/isps/results/elevatefiber_result.json", "r") as f:
...
result = agent.run(
    ...
    save_content_dir="examples/isps/save/elevatefiber",
    session="elevatefiber"
)
...
with open("examples/isps/results/elevatefiber_result.json", "w") as f:
