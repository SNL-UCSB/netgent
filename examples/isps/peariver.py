...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Pea River CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://peariver.crowdfiber.com/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Pea River page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Type `%address%` into the input field", "Type `%zip_code%` into the input field", "Click the 'Service Type' dropdown, press down key and enter", "Click the 'Go' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
    ...
    StatePrompt(
        ...
        triggers=["If you see 'Pea River is in your neighborhood. Order now!'"],
        actions=[
...
# Pick an address (using same index approach as user's snippet logic with valid range check if needed, but sticking to logic)
...
result = agent.run(
    ...
    session="peariver",
    save_content_dir="examples/isps/save/peariver"
)

# agent.set_state_wait_time(5) # Optional, kept from snippet logic if needed but commented out as previous was just setting it

input("Press Enter to continue...")

# Write result to file
os.makedirs("examples/isps/results", exist_ok=True)
with open("examples/isps/results/peariver_result.json", "w") as f:
