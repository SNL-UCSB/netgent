...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to LCUB CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://join.lcub.com/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the LCUB page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Accept cookies first", "Type `%address%` into the input field", "Type `%zip_code%` into the input field", "Click on 'Service Type' dropdown, down arrow key once, then enter key", "Click the 'Go' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        ...
        actions=["Scroll all the way down", "Click the 'Next' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="SERVICEABLE",
...
result = agent.run(
    ...
    session="lcub",
    save_content_dir="examples/isps/save/lcub"
)
...
with open("examples/isps/results/lcub_result.json", "w") as f:
