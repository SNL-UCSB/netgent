...
prompts = [
        StatePrompt(
            name="Navigate To Cox",
            description="Navigated to the Cox Website",
            triggers=["If it is on the chrome homescreen"],
            actions=["Go to https://www.cox.com/residential/internet.html word for word", "TERMINATE AT THIS POINT"],
        ),
        StatePrompt(
            name="Enter Address on Cox Homepage",
            description="Enter the address to check service availability",
            triggers=["If on the Cox homepage with an address input field"],
        ...
        StatePrompt(
            ...
            description="Handle the question about existing Cox customer",
            triggers=["If asked 'Are you an existing Cox customer?' or similar"],
        ...
        StatePrompt(
            name="Internet Available at Address on Cox",
        ...
        StatePrompt(
            name="Internet Not Available at Address on Cox",
...
with BQTDatabase() as db:
    # Fetching addresses for Cox service area
    rows = db.query("SELECT * FROM bqtplus.cox_addresses LIMIT 1000")
    for row in rows:
...
result = agent.run(
    ...
    save_content_dir="examples/isps/save/cox",
    session="cox"
)

# Write result to file
os.makedirs("examples/isps/results", exist_ok=True)
with open("examples/isps/results/cox_result.json", "w") as f:
...
try:
    with open("examples/isps/results/cox_result.json", "r") as f:
        result = json.load(f)
except FileNotFoundError:
    result = []

result = agent.run(state_prompts=prompts, state_repository=result, save_content_dir="examples/isps/save/cox", session="cox")
