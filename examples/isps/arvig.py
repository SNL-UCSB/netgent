...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Arvig CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://connect.arvig.com/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Arvig page",
    ...
    StatePrompt(
        ...
        triggers=["If you see 'Verify that the marker below is the correct location for your address. If not, drag the marker to your actual location before continuing.'"],
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available - (e.g. 'We're sorry, your location isn't currently in our database. However, wireless service may be available.')",
        triggers=["If you see 'We're sorry, your location isn't currently in our database. However, wireless service may be available.'"],
        actions=["DON'T DO ANYTHING, JUST TERMINATE AT THIS POINT"],
...
if os.path.exists("examples/isps/results/arvig_result.json"):
    with open("examples/isps/results/arvig_result.json", "r") as f:
        try:
            data = json.load(f)
...
for i in range(15):
    try:
        ...
        result = agent.run(
            ...
            session="arvig",
            save_content_dir="examples/isps/save/arvig",
            close_browser=True
        )
...
with open("examples/isps/results/arvig_robust_results.json", "w") as f:
    json.dump(results, f, indent=2)

# Update the main result repository
with open("examples/isps/results/arvig_result.json", "w") as f:
