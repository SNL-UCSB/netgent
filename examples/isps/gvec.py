...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to GVEC shop page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://shop.gvec.net/#/order/1 word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
    ...
    StatePrompt(
        ...
        triggers=["If you see 'Show Us Your Interest!' or 'URL: https://www.gvec.net/internet-interest/'"],
...
if os.path.exists("examples/isps/results/gvec_result.json"):
    with open("examples/isps/results/gvec_result.json", "r") as f:
        try:
            data = json.load(f)
...
result = agent.run(
    ...
    session="gvec",
    save_content_dir="examples/isps/save/gvec"
)
...
with open("examples/isps/results/gvec_result.json", "w") as f:
