...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to CAEC Access CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://caecaccess.com/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the CAEC Access page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Scroll down", "Type `%address%` into the input field", "Type `%zip_code%` into the input field", "Press on the 'Select Market' dropdown menu and press down key and press enter key", "Click the 'Go' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available - (e.g. 'Unfortunately, your address is not in an area where service is available at this time. However, if we receive enough expressions of interest in your neighborhood, we may add this area in the future. To register your interest, please complete the form below. This does not obligate you for service, but will allow us to keep you informed of changes.')",
...
if os.path.exists("examples/isps/results/caecaccess_result.json"):
    with open("examples/isps/results/caecaccess_result.json", "r") as f:
        try:
            data = json.load(f)
...
for i in range(15):
    try:
        ...
        result = agent.run(
            ...
            session="caecaccess",
            save_content_dir="examples/isps/save/caecaccess",
            close_browser=True
        )
...
with open("examples/isps/results/caecaccess_robust_results.json", "w") as f:
    json.dump(results, f, indent=2)

# Update the main result repository
with open("examples/isps/results/caecaccess_result.json", "w") as f:
