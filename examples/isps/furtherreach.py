...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Further Reach homepage",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://furtherreach.net/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="HOMEPAGE",
        description="If on the Further Reach homepage",
        triggers=["If you see 'Check Availability' button or link"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Type `%address%` and `%zip_code%` into the input field", "Click 'Check Availability'", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="SERVICEABLE",
...
result = agent.run(
    ...
    session="furtherreach",
    save_content_dir="examples/isps/save/furtherreach"
)
...
with open("examples/isps/results/furtherreach_result.json", "w") as f:
