...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Quantum Fiber shop page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://www.quantumfiber.com/shop word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="Enter address to check availability",
        triggers=["If you see 'Enter your address to check availability.'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Click the 'My street address' input field", "Type `%address%` into the input field", "Click the 'Yes' text or button", "Click the 'Check Availability' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="SERVICEABLE",
        description="Service available",
        triggers=["If you see 'Congrats'"],
    ...
    StatePrompt(
        ...
        triggers=["If you see 'Our records show there may already be service at this address.'"],
        actions=["TERMINATE AT THIS POINT"],
    ...
    StatePrompt(
        ...
        triggers=["If you see 'We\\'re not available at your address yet.'"],
        actions=["TERMINATE AT THIS POINT"],
        end_state="no_service"
    ),
    StatePrompt(
        name="ACCESS_DENIED",
        description="Bot detection or access denied",
...
with BQTDatabase() as db:
    # Fetching addresses for Quantum Fiber service area
    rows = db.query("SELECT * FROM bqtplus.quantum_addresses LIMIT 1000")
    for row in rows:
...
try:
    with open("examples/isps/results/quantumfiber_result.json", "r") as f:
...
with open("examples/isps/results/quantumfiber_result.json", "w") as f:
