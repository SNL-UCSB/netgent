...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Co-Mo Connect CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://join.co-mo.net/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Co-Mo Connect page",
...
if os.path.exists("examples/isps/results/como_result.json"):
    with open("examples/isps/results/como_result.json", "r") as f:
        try:
            data = json.load(f)
...
for i in range(15):
    try:
        ...
        result = agent.run(
            ...
            session="como",
            save_content_dir="examples/isps/save/como",
            close_browser=True
        )
...
with open("examples/isps/results/como_robust_results.json", "w") as f:
    json.dump(results, f, indent=2)

# Update the main result repository
with open("examples/isps/results/como_result.json", "w") as f:
