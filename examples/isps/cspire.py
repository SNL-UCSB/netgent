...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to C Spire CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://interest.cspire.com/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the C Spire page",
...
if os.path.exists("examples/isps/results/cspire_result.json"):
    with open("examples/isps/results/cspire_result.json", "r") as f:
        try:
            data = json.load(f)
...
result = agent.run(
    ...
    session="cspire",
    save_content_dir="examples/isps/save/cspire"
)
...
with open("examples/isps/results/cspire_result.json", "w") as f:
