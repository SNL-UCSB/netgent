...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to CCECA CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://connect.cceca.net/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the CCECA page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Accept cookies first", "Type `%address%` into the input field", "Type `%zip_code%` into the input field", "Press on the dropdown, press down arrow three times and press enter", "Click the 'Go' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
...
if os.path.exists("examples/isps/results/cceca_result.json"):
    with open("examples/isps/results/cceca_result.json", "r") as f:
        try:
            data = json.load(f)
...
for i in range(15):
    try:
        ...
        result = agent.run(
            ...
            session="cceca",
            save_content_dir="examples/isps/save/cceca",
            close_browser=True
        )
...
with open("examples/isps/results/cceca_robust_results.json", "w") as f:
    json.dump(results, f, indent=2)

# Update the main result repository
with open("examples/isps/results/cceca_result.json", "w") as f:
