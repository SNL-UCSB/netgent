...
prompts = [
    ...
    StatePrompt(
        ...
        triggers=["Enter your address below to see if we can service your home or business!'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Type `%address%` into the input field", "Type `%zip_code%` into the input field", "Click on 'Select Market' dropdown, down arrow key once, then enter key", "Click the 'Go' button", "TERMINATE AT THIS POINT AND DO NOT DO ANYTHING ELSE"],
    ),
    StatePrompt(
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available - (e.g. 'We've sorry we do not current serve your area.')",
        triggers=["If you see 'We've sorry we do not current serve your area.'"],
        actions=["TERMINATE AT THIS POINT AND DO NOT DO ANYTHING ELSE"],
...
with open("examples/isps/results/infinium_robust_results.json", "w") as f:
