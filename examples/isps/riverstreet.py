...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to the RiverStreet Services Check Availability page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://myriverstreet.net/services/internet/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
...
with BQTDatabase() as db:
    # Fetching addresses for RiverStreet service area
    rows = db.query("SELECT * FROM bqtplus.riverstreet_addresses LIMIT 1000")
    for row in rows:
...
with open("examples/isps/results/riverstreet_result.json", "w") as f:
