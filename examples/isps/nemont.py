...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Nemont CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://connect.nemont.com/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Nemont page",
    ...
    StatePrompt(
        ...
        triggers=["If you see 'Please verify that your 911 address is in the correct location on the map below. Then, click the Next button to see if services are available at'"],
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available - (e.g. 'Not in the Nemont service area.')",
        triggers=["If you see 'Not in the Nemont service area.'"],
        actions=["DON'T DO ANYTHING, JUST TERMINATE AT THIS POINT"],
...
if os.path.exists("examples/isps/results/nemont_result.json"):
    with open("examples/isps/results/nemont_result.json", "r") as f:
        try:
            data = json.load(f)
...
result = agent.run(
    ...
    session="nemont",
    save_content_dir="examples/isps/save/nemont"
)
...
with open("examples/isps/results/nemont_result.json", "w") as f:
