...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Columbia iNet page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://service.columbiainet.com/front_end word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Columbia iNet page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Type `%address%` into the input field", "Type `%zip_code%` into the input field", "Click the 'Go' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available - (e.g. 'Make It a Possibility.')",
        triggers=["If you see 'Make It a Possibility.'"],
        actions=["DON'T DO ANYTHING, JUST TERMINATE AT THIS POINT"],
...
if os.path.exists("examples/isps/results/columbiainet_result.json"):
    with open("examples/isps/results/columbiainet_result.json", "r") as f:
        try:
            data = json.load(f)
...
for i in range(15):
    try:
        ...
        result = agent.run(
            ...
            session="columbiainet",
            save_content_dir="examples/isps/save/columbiainet",
            close_browser=True
        )
...
with open("examples/isps/results/columbiainet_robust_results.json", "w") as f:
    json.dump(results, f, indent=2)

# Update the main result repository
with open("examples/isps/results/columbiainet_result.json", "w") as f:
