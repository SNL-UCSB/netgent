...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Go Energy Net CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://goenergynet.com/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Go Energy Net page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Type `%address%` into the input field", "Press 'down arrow' key, 'enter' key to submit", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="SERVICEABLE",
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available (e.g. 'Aww shucks! Our service isn't available' or 'Map is not available')",
        triggers=["If you see 'Aww shucks! Our service isn't available' or 'Map is not available'"],
        actions=["TERMINATE AT THIS POINT"],
...
if os.path.exists("examples/isps/results/goenergynet_result.json"):
    with open("examples/isps/results/goenergynet_result.json", "r") as f:
        try:
            data = json.load(f)
...
for i in range(15):
    try:
        ...
        result = agent.run(
            ...
            session="goenergynet",
            save_content_dir="examples/isps/save/goenergynet",
            close_browser=True
        )
...
with open("examples/isps/results/goenergynet_robust_results.json", "w") as f:
    json.dump(results, f, indent=2)

# Update the main result repository
with open("examples/isps/results/goenergynet_result.json", "w") as f:
