...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Socket page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://shop.socket.net/#/order/1 word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Socket page",
...
try:
    with open("examples/isps/results/socket_result.json", "r") as f:
...
result = agent.run(
    ...
    save_content_dir="examples/isps/save/socket-isp",
    session="socket-isp"
)
...
with open("examples/isps/results/socket_result.json", "w") as f:
