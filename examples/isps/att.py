...
prompts = [
        StatePrompt(
            name="Navigate To AT&T",
            description="Navigated to the AT&T Website",
            triggers=["If it is on the chrome homescreen"],
            actions=["Go to https://www.att.com/internet/", "TERMINATE AT THIS POINT"],
        ),
        StatePrompt(
            name="Enter Address on AT&T Homepage",
            description="Enter the address to check service availability",
            triggers=["If on the AT&T homepage with an address input field"],
            actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Scroll down to the address input field", "Click the address input field", "Type `%address%` into the address input field", "Wait for address suggestions to appear", "Press down key to select the first suggestion", "You must press the submit button to confirm", "TERMINATE AT THIS POINT"],
        ),
        StatePrompt(
            name="Handle Existing Customer Question",
            description="Handle the question about existing AT&T customer",
            triggers=["If asked 'Are you an existing AT&T customer?' or similar"],
        ...
        StatePrompt(
            name="Internet Available at Address on AT&T",
        ...
        StatePrompt(
            name="Internet Not Available at Address on AT&T",
...
with BQTDatabase() as db:
    # Fetching addresses for AT&T service area
    rows = db.query("SELECT * FROM bqtplus.att_addresses LIMIT 1000")
    for row in rows:
...
result = agent.run(
    ...
    save_content_dir="examples/isps/save/att",
    session="att"
)

# Write result to file
os.makedirs("examples/isps/results", exist_ok=True)
with open("examples/isps/results/att_result.json", "w") as f:
...
try:
    with open("examples/isps/results/att_result.json", "r") as f:
        result = json.load(f)
except FileNotFoundError:
    result = []

result = agent.run(state_prompts=prompts, state_repository=result, save_content_dir="examples/isps/save/att", session="att")
