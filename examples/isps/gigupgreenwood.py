...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to GigUp Greenwood CrowdFiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://join.gigupgreenwood.com/front_end/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the GigUp Greenwood page",
    ...
    StatePrompt(
        name="NO_SERVICE",
        description="Service not available - (e.g. 'Unfortunately, we are not currently in your area.')",
        triggers=["If you see 'Unfortunately, we are not currently in your area.'"],
        actions=["TERMINATE AT THIS POINT"],
...
if os.path.exists("examples/isps/results/gigupgreenwood_result.json"):
    with open("examples/isps/results/gigupgreenwood_result.json", "r") as f:
        try:
            data = json.load(f)
...
for i in range(15):
    try:
        ...
        result = agent.run(
            ...
            session="gigupgreenwood",
            save_content_dir="examples/isps/save/gigupgreenwood",
            close_browser=True
        )
...
with open("examples/isps/results/gigupgreenwood_robust_results.json", "w") as f:
    json.dump(results, f, indent=2)

# Update the main result repository
with open("examples/isps/results/gigupgreenwood_result.json", "w") as f:
