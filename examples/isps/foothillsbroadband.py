...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Foothills Broadband page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://ready4fiber.foothillsbroadband.net/signup word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="Enter address to check availability",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Press Enter Address Button", "Type `%address%` into the input field", "Type `%zip_code%` into the input field", "press 'select market' button", "press down and enter to confirm selection", "Click the 'Go' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
    ...
    StatePrompt(
        ...
        triggers=["If you see 'Foothills Broadband is in your neighborhood. Order now!'"],
        actions=[
...
try:
    with open("examples/isps/results/foothillsbroadband_result.json", "r") as f:
...
result = agent.run(
    ...
    save_content_dir="examples/isps/save/foothillsbroadband",
    session="foothillsbroadband"
)
...
with open("examples/isps/results/foothillsbroadband_result.json", "w") as f:
