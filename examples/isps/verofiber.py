...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to Vero Fiber page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://www.verofiber.com/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the Vero Fiber page",
        triggers=["If you see 'Please enter your address below'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Press 'Check Address' button", "Type `%address%` into the input field", "Press down and enter to select the first option", "Click 'Startc' to confirm selection", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
    ...
    StatePrompt(
        ...
        triggers=["If you see 'Vero Fiber is in your neighborhood. Order now!'"],
        actions=[
...
try:
    with open("examples/isps/results/verofiber_result.json", "r") as f:
...
result = agent.run(
    ...
    save_content_dir="examples/isps/save/verofiber",
    session="verofiber"
)
...
with open("examples/isps/results/verofiber_result.json", "w") as f:
