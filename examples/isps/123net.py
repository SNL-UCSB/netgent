...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to 123NET sign up page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://www.123.net/home-fiber-internet/sign-up/ word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="Enter address to check availability",
        triggers=["If you see 'See Pricing In Your Area'"],
        actions=["FOLLOW THESE INSTRUCTIONS CLOSELY", "Click the 'Your Address' input field", "Type `%address%` into the input field", "Press down key to select the first suggestion", "Press enter key to confirm selection", "Click the 'Select 250 Mbps' text or radio button", "Click the 'Continue' button", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="NO_SERVICE",
...
with BQTDatabase() as db:
    # Fetching addresses for 123NET service area
    rows = db.query("SELECT * FROM bqtplus.net123_addresses LIMIT 1000")
    for row in rows:
