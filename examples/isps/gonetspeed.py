...
prompts = [
    StatePrompt(
        name="START",
        description="Navigate to GoNetspeed page",
        triggers=["If it is on the chrome homescreen"],
        actions=["Go to https://shop.gonetspeed.com/#/order/1 word for word", "TERMINATE AT THIS POINT"],
    ),
    StatePrompt(
        name="ADDRESS_BAR",
        description="If on the GoNetspeed page",
    ...
    StatePrompt(
        ...
        triggers=["If you see 'GoNetspeed is in your neighborhood. Order now!'"],
        actions=[
...
try:
    with open("examples/isps/results/gonetspeed_result.json", "r") as f:
...
result = agent.run(
    ...
    save_content_dir="examples/isps/save/gonetspeed",
    session="gonetspeed"
)
...
with open("examples/isps/results/gonetspeed_result.json", "w") as f:
